...
def compute_score_modify(logits, labels):
...
def compute_scaler_score_modify(logits,labels):
...
def train_for_epoch(opt,model,train_loader,test_loader):
                ...
                if 'encoder.layer' in n:
                    try:
                        ...
                        if 'encoder.layers' in n:
                            layer_num = int(n[n.find('encoder.layers') + 15:].split('.')[0])
                        else:
                            layer_num = int(n[n.find('encoder.layer') + 14:].split('.')[0])
                    except:
                        print(n)
    ...
    for epoch in range(opt.EPOCHS):
        ...
        for i,batch in enumerate(train_loader):
            ...
            cap_tokens = batch['cap_tokens'].long().cuda()
            sent_tokens = batch['sent_tokens'].long().cuda()
            label=batch['label'].float().cuda().view(-1,1)
            cap_mask=batch['cap_mask'].cuda()
            sent_mask=batch['sent_mask'].cuda()
            target=batch['target'].cuda()
            img_feat = batch["img_feat"].float().cuda()
            labels = batch["label"].unsqueeze(1).float().cuda() 
            feat=None
            
            loss_temp = 0
            
            if opt.MODEL=='pbm':
                logits, loss_temp = model(cap_tokens,sent_tokens,cap_mask,sent_mask,img_feat,labels,feat)
                if opt.FINE_GRIND:
                ...
                logits=model(cap_tokens,cap_mask,feat)
   
            loss = loss_temp
            batch_score = compute_score_modify(logits,labels)
...
def eval_multi_model(opt,model, tokenizer):
    ...
    for k in range(num_queries):
        ...
        for i,batch in enumerate(test_loader):
            with torch.no_grad():
                ...
                logits, _ =model(cap_tokens,sent_tokens,cap_mask,sent_mask,img_feat,labels,feat)
                if opt.FINE_GRIND:
                ...
                norm_prob = torch.sigmoid(logits)
                norm_logits=norm_prob[:,0].unsqueeze(-1)
                bz=cap_tokens.shape[0]
                for j in range(bz):
                    cur_img=img[j]
    ...
    scores=compute_scaler_score_modify(probs,labels)
